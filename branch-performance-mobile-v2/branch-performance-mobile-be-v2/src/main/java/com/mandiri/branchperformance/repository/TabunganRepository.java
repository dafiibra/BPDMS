...
@Repository
public interface TabunganRepository extends JpaRepository<FactTabunganDaily, UUID> {

    // ── Latest snapshot ───────────────────────────────────────────────────────

    @Query(value = """
        SELECT MAX(SNAPSHOT_DATE) FROM FACT_TABUNGAN_DAILY
        WHERE BRANCH_ID = HEXTORAW(REPLACE(:branchId, '-', ''))
        """, nativeQuery = true)
    Timestamp findLatestSnapshotByBranch(@Param("branchId") String branchId);

    @Query(value = """
        SELECT MAX(d.SNAPSHOT_DATE) FROM FACT_TABUNGAN_DAILY d
    ...
    @Query(value = """
        SELECT d.* FROM FACT_TABUNGAN_DAILY d
        WHERE d.SNAPSHOT_DATE = :snapshotDate
          AND d.BRANCH_ID = HEXTORAW(REPLACE(:branchId, '-', ''))
        """, nativeQuery = true)
    List<FactTabunganDaily> findByBranchAndSnapshot(@Param("branchId") String branchId,
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    List<FactTabunganDaily> findByAreaAndSnapshot(@Param("areaId") String areaId,
                                                   @Param("snapshotDate") LocalDate snapshotDate);
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    List<FactTabunganDaily> findByRegionAndSnapshot(@Param("regionId") String regionId,
                                                     @Param("snapshotDate") LocalDate snapshotDate);
    ...
    // ── Prev-year history for 4-series chart ──────────────────────────────────
    ...
    @Query(value = """
        SELECT * FROM (
            ...
            FROM FACT_TABUNGAN_DAILY d
            WHERE d.BRANCH_ID = HEXTORAW(REPLACE(:branchId, '-', ''))
              AND EXTRACT(YEAR FROM d.SNAPSHOT_DATE) = EXTRACT(YEAR FROM SYSDATE) - 1
            GROUP BY d.SNAPSHOT_DATE
            ORDER BY d.SNAPSHOT_DATE DESC
        ) WHERE ROWNUM <= 12
        """, nativeQuery = true)
    List<Object[]> findPrevYearHistoryByBranch(@Param("branchId") String branchId);
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    List<Object[]> findPrevYearHistoryByArea(@Param("areaId") String areaId);
    ...
    @Query(value = """
        ...
        """, nativeQuery = true)
    List<Object[]> findPrevYearHistoryByRegion(@Param("regionId") String regionId);

    // ── Metrics: 5-day delta and retail proportion ────────────────────────────

    @Query(value = """
        SELECT SUM(d.ENDING_BALANCE) FROM FACT_TABUNGAN_DAILY d
        JOIN MST_SEGMENTS m ON d.SEGMENT_ID = m.ID
        WHERE d.BRANCH_ID = HEXTORAW(REPLACE(:branchId, '-', ''))
          AND d.SNAPSHOT_DATE = :snapshotDate
          AND UPPER(m.SEGMENT_CODE) = 'RETAIL'
        """, nativeQuery = true)
    Object findRetailEndBalByBranchAndSnapshot(@Param("branchId") String branchId,
                                               @Param("snapshotDate") LocalDate snapshotDate);
    ...
    @Query(value = """
        ...
        WHERE b.AREA_ID = HEXTORAW(REPLACE(:areaId, '-', ''))
          ...
          AND UPPER(m.SEGMENT_CODE) = 'RETAIL'
        """, nativeQuery = true)
    Object findRetailEndBalByAreaAndSnapshot(@Param("areaId") String areaId,
                                             @Param("snapshotDate") LocalDate snapshotDate);
    ...
    @Query(value = """
        ...
        WHERE a.REGION_ID = HEXTORAW(REPLACE(:regionId, '-', ''))
          ...
          AND UPPER(m.SEGMENT_CODE) = 'RETAIL'
        """, nativeQuery = true)
    Object findRetailEndBalByRegionAndSnapshot(@Param("regionId") String regionId,
                                               @Param("snapshotDate") LocalDate snapshotDate);
}
